...
async function loadPokemon(id) {
    try {
        ...
        console.error("an error accured while fetching pokemon data", error);
        return false;
    }
}
...
// Set background colors based on Pokemon's primary type
...
function setTypeBackgroundColor(pokemon) {
    ...
    if (!color) {
        console.warn(`color not defined for: ${mainType}`);
        return;
    }
...
function displayPokemonDetails(pokemon) {
    ...
    const detailMainElement = document.querySelector(".detail-main")
    if (currentPokemonClass) {
        detailMainElement.classList.remove(currentPokemonClass);
    }

    currentPokemonClass = name.toLowerCase();
    detailMainElement.classList.add(currentPokemonClass);
    ...
    const favoriteCheckbox = document.querySelector("#favorite-detail-checkbox");

    favoriteCheckbox.setAttribute("data-pokemon-id", id);
    favoriteCheckbox.checked = isFavorite(id);
    const newCheckbox = favoriteCheckbox.cloneNode(true);
    favoriteCheckbox.parentNode.replaceChild(newCheckbox, favoriteCheckbox);
    document.querySelector("#favorite-detail-checkbox").addEventListener("change", (e) => {
...
function toggleFavorite(id, isChecked) {
    ...
    if (isChecked) {
        ...
        favorites = favorites.filter(fav => fav !== String(id));
    }
